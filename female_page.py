...
def display_prompt_result():
    number1 = st.session_state['female_numbers'][0]
    number2 = st.session_state['female_numbers'][1]
    number3 = st.session_state['female_numbers'][2]
    number4 = st.session_state['female_numbers'][3]
    selected_position = st.session_state['female_numbers'][4]

    prompt = f"""
Please give me a female football player that fits the following data (or is close to it): 
...
Please give me only one player. It is okay if not all of the data perfectly matches, just the closer the better. 
I know that you can not give me the exact up-to-date data, but just shout me a player anyways. If you can't find anyone just write: "Can't find a player that fits tha data."
...
def function_female():
    ...
    with tabs1:
        ...
        initial_values = [27, 165, 60, 27, "Goalkeeper"]
        min_values = [16, 150, 50, 22, None]
        max_values = [40, 190, 85, 33, None]
        ...
        if 'female_numbers' not in st.session_state:
            st.session_state['female_numbers'] = initial_values[:]
        if 'response' not in st.session_state:
            st.session_state['response'] = ""

        for i in range(5):
            if names[i] == "Position":
                st.session_state['female_numbers'][i] = st.selectbox(
                    f"{names[i]}:",
                    options=positions,
                    index=positions.index(st.session_state['female_numbers'][i]),
                    key=f"selectbox_{i}"
                )
            else:
                ...
                with col1:
                    if st.button(f"▲", key=f"up{i}"):
                        if st.session_state['female_numbers'][i] < max_values[i]:
                            st.session_state['female_numbers'][i] += 1

                with col2:
                    if st.button(f"▼", key=f"down{i}"):
                        if st.session_state['female_numbers'][i] > min_values[i]:
                            st.session_state['female_numbers'][i] -= 1

                with col3:
                    st.write(f"{names[i]}: {st.session_state['female_numbers'][i]}")
